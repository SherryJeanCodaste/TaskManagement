...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-6 bg-[#F9FAFB]">
            <!-- Welcome Header -->
            <div class="bg-gradient-to-r from-[#5B21B6] to-[#7C3AED] rounded-2xl p-8 text-white shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-3xl font-bold mb-2">
                            {{ props.auth.user.login_count <= 1 ? 'Welcome' : 'Welcome back' }}, {{ props.auth.user.name }}!
                        </h1>
                        <p class="text-white/90">{{ roleDisplay }} • {{ isCustomer ? 'Create and track your tasks' : 'View and manage your assigned tasks' }}</p>
                    </div>
                ...
                <div class="bg-white rounded-xl p-6 border border-[#CBD5E1] shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-[#5B21B6] mt-2">{{ props.stats.total_tasks }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl p-6 border border-[#CBD5E1] shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-[#EF4444] mt-2">{{ props.stats.pending }}</p>
                        </div>
                        <div class="w-12 h-12 bg-[#EF4444]/10 rounded-lg flex items-center justify-center">
                ...
                <div class="bg-white rounded-xl p-6 border border-[#CBD5E1] shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-[#F97316] mt-2">{{ props.stats.in_progress }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl p-6 border border-[#CBD5E1] shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-[#22C55E] mt-2">{{ props.stats.completed }}</p>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-xl border border-[#CBD5E1] shadow-sm p-6">
                    <h3 class="text-lg font-bold text-[#1E293B] mb-4">Quick Actions</h3>
                    <div class="space-y-3">
                        <a :href="'/tasks'" class="flex items-center gap-3 p-3 rounded-lg hover:bg-[#F9FAFB] transition-colors group">
                            <div class="w-10 h-10 bg-[#5B21B6]/10 rounded-lg flex items-center justify-center group-hover:bg-[#5B21B6]/20 transition-colors">
                                <svg class="w-5 h-5 text-[#5B21B6]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                                </svg>
                            </div>
                            <div>
                                <p class="font-semibold text-[#1E293B]">View All Tasks</p>
                                <p class="text-xs text-[#64748B]">{{ isCustomer ? 'Manage your assignments' : 'Manage your assignments' }}</p>
                            </div>
                        </a>
                        <a :href="'/projects'" class="flex items-center gap-3 p-3 rounded-lg hover:bg-[#F9FAFB] transition-colors group">
                            <div class="w-10 h-10 bg-[#06B6D4]/10 rounded-lg flex items-center justify-center group-hover:bg-[#06B6D4]/20 transition-colors">
                                <svg class="w-5 h-5 text-[#06B6D4]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path>
                                </svg>
                            </div>
                            <div>
                                <p class="font-semibold text-[#1E293B]">My Projects</p>
                ...
                <!-- My Projects -->
                <div class="lg:col-span-2 bg-white rounded-xl border border-[#CBD5E1] shadow-sm p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-bold text-[#1E293B]">My Projects</h3>
                        <a :href="'/projects'" class="text-[#5B21B6] hover:text-[#6D28D9] font-semibold text-sm">View All →</a>
                    </div>
                    <div v-if="(isCustomer ? projects : myProjects) && (isCustomer ? projects : myProjects).length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <a v-for="project in (isCustomer ? projects : myProjects).slice(0, 4)" :key="project.id" :href="`/projects/${project.id}`" class="border border-[#E2E8F0] rounded-lg p-4 hover:border-[#5B21B6] hover:shadow-md transition-all group">
                            <div class="flex items-start gap-3">
                                <div class="w-10 h-10 bg-[#5B21B6]/10 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-[#5B21B6]/20 transition-colors">
                                    <svg class="w-5 h-5 text-[#5B21B6]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path>
                                    </svg>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="font-semibold text-[#1E293B] truncate">{{ project.name }}</p>
                    ...
                    <div v-else class="text-center py-8 text-[#94A3B8]">
                        ...
                        <p class="text-sm mb-4">{{ isCustomer ? 'No projects yet' : 'No projects assigned yet' }}</p>
                        <a v-if="isCustomer" :href="'/projects/create'" class="inline-block bg-[#5B21B6] hover:bg-[#6D28D9] text-white font-semibold px-6 py-2 rounded-lg transition-colors">
                            Create Project
                        </a>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl border border-[#CBD5E1] shadow-sm overflow-hidden">
                <div class="p-6 border-b border-[#CBD5E1] flex items-center justify-between">
                    <div>
                        <h2 class="text-xl font-bold text-[#1E293B]">{{ isCustomer ? 'Your Recent Tasks' : 'Your Assigned Tasks' }}</h2>
                        <p class="text-sm text-[#1E293B]/60 mt-1">{{ isCustomer ? 'Tasks you have created' : 'Tasks assigned to you' }}</p>
                    </div>
                        ...
                        <tbody class="divide-y divide-[#CBD5E1]">
                            <tr v-for="task in displayTasks" :key="task.id" @click="router.visit(`/tasks/${task.id}`)" class="hover:bg-[#F9FAFB] transition-colors cursor-pointer">
                                <td class="px-6 py-4">
                                    ...
                                    <div v-if="task.description" class="text-sm text-[#1E293B]/60 mt-1">{{ task.description?.substring(0, 50) }}{{ task.description?.length > 50 ? '...' : '' }}</div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-[#1E293B]/60 text-sm">{{ new Date(task.created_at).toLocaleDateString() }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div v-else class="p-12 text-center">
                    ...
                    <p class="text-[#1E293B]/60">{{ isCustomer ? 'No tasks created yet. Create your first task!' : 'No tasks assigned to you yet.' }}</p>
                    <a v-if="isCustomer" :href="'/tasks/create'" class="inline-block mt-4 px-6 py-2 bg-[#5B21B6] text-white rounded-lg hover:bg-[#6D28D9] transition-colors font-semibold">
                        Create Task
                    </a>
                </div>
            </div>
