    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4 bg-[#F9FAFB]">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-[#1E293B]">Review Tasks</h1>
                    <p class="text-[#1E293B] mt-1 text-sm">Review all tasks before submitting</p>
                </div>
                <button
                    @click="goBack"
                    class="px-3 py-2 text-[#1E293B] hover:bg-white rounded-lg transition-colors border border-[#CBD5E1] text-sm"
                >
                    ← Back to Add More
                </button>
            </div>
            ...
            <div class="bg-white rounded-xl border border-[#CBD5E1] shadow-sm p-4">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-[#5B21B6]/10 flex items-center justify-center">
                        <svg class="w-5 h-5 text-[#5B21B6]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path>
                        </svg>
                    </div>
                    <div>
                        <p class="text-xs text-[#64748B]">Project</p>
                        <p class="text-base font-semibold text-[#1E293B]">{{ projectName }}</p>
                    </div>
                    <div class="ml-auto">
                        <p class="text-xs text-[#64748B]">Total Tasks</p>
                        <p class="text-xl font-bold text-[#5B21B6]">{{ tasks.length }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-3">
                <div v-for="(task, index) in tasks" :key="task.id" 
                    class="bg-white rounded-xl border border-[#CBD5E1] shadow-sm p-4 hover:shadow-md transition-shadow">
                    <div class="flex items-start justify-between mb-2">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-lg bg-[#F1F5F9] flex items-center justify-center">
                                <span class="text-sm font-bold text-[#64748B]">{{ index + 1 }}</span>
                            </div>
                            <div>
                                <h3 class="text-base font-semibold text-[#1E293B]">{{ task.title }}</h3>
                                <span class="inline-block mt-1 px-2 py-0.5 text-xs font-medium rounded-full border capitalize"
                    ...
                    <div v-if="task.description" class="mt-2 pl-11">
                        <p class="text-sm text-[#64748B] leading-relaxed">{{ task.description }}</p>
                    </div>
                    <div v-else class="mt-2 pl-11">
            ...
            <div class="bg-white rounded-xl border border-[#CBD5E1] shadow-sm p-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs text-[#64748B] mb-1">Ready to submit?</p>
                        <p class="text-sm text-[#1E293B]">All tasks will be automatically assigned to the appropriate developers</p>
                    </div>
                    <button
                        @click="submit"
                        :disabled="form.processing"
                        class="px-6 py-2 bg-[#5B21B6] text-white rounded-lg hover:bg-[#6D28D9] transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                    >
